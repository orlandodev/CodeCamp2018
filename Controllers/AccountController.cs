...
namespace TwoFactorAuthDemo.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        public async Task<ActionResult> VerifyOTP(string provider, string returnUrl, bool rememberMe)
        {
            // Require that the user has already logged in via username/password or external login
            if (!await SignInManager.HasBeenVerifiedAsync())
            {
            ...
            return View(new VerifyOTPViewModel { Provider = provider, ReturnUrl = returnUrl, RememberMe = rememberMe });
        }
        ...
        public async Task<ActionResult> SendCode(string returnUrl, bool rememberMe)
        {
            ...
            {
                if(factor.StartsWith("Google Authenticator"))
                {
